...
### В чате копируются старые сообщения "Authorization successful!"

Бот удаляет предыдущее сообщение об авторизации перед отправкой нового. Для этого в Supabase должна быть колонка `last_bot_message_id` в таблице `users`. Выполните миграцию в Supabase (SQL Editor):

```sql
ALTER TABLE users ADD COLUMN IF NOT EXISTS last_bot_message_id INTEGER;
```

После этого при каждой новой авторизации будет удаляться предыдущее такое сообщение, в чате останется только одно.
